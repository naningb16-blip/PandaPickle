-- Migration: Add security indexes and constraints for duplicate prevention
...
CREATE INDEX idx_user_court_date_time 
...
CREATE INDEX idx_registrations_created 
...
CREATE INDEX idx_session_status_preference 
ON open_play_registrations(session_id, status, match_preference, friend_group);

-- Optimize email uniqueness checks (already exists as UNIQUE, but explicit index helps)
-- Note: UNIQUE constraint already creates an index, this is just documentation

-- Optimize session date queries
CREATE INDEX idx_session_date_status 
